...
const ProfessionalLists = () => {
  ...
  const getProfessionalByType = () => {
    axiosInstance.get(`${API_URLS.psychicCategory}type?type=${queryParams.get('type')}&value=${queryParams.get('value').toLowerCase()}`)
      .then((res) => {
        console.log("RES", res);
        if (res) {
          ...
          <div className="home_sec3">
            <section className='explore_category_sec'>
              <div className='container'>
                <h2>{`Explore ${queryParams.get('type')} Psychics`}</h2>
                <div className='row'>
